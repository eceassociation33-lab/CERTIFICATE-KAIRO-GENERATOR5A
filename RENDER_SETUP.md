...
| Key | Value |
| :--- | :--- |
| `NODE_ENV` | `production` (CRITICAL: Otherwise the app won't serve correctly) |
...
> [!IMPORTANT]
> Render will automatically assign a **PORT**. Our updated code now handles this automatically. Ensure `NODE_ENV` is set to `production` so the server serves the pre-built frontend from the `dist` folder.
